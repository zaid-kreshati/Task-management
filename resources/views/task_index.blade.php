    ...
    <body>
        ...
        @role('leader')
            ...
            <form id="taskForm" action="{{ route('tasks.store') }}" method="POST" style="display: none;">
                ...
                <div class="coupon">
                    ...
                    <div class="task-list2">
                        ...
                        <div id="discountedPrice" style="margin-top: 20px;">
                            <p>{{ __('Price after discount:') }} <span id="priceValue">{{ getPrice() }}</span></p>
                        </div>
                ...
                <!-- Submit button to trigger both creation and payment -->
                <div>
                <button type="button" id="proceedToPayment" class="btn btn-primary">{{ __('Proceed to Payment') }}</button>
                </div>
            </form>
        @endrole
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    // Redirect to payment after task creation with the final price


                    alert(response.newPrice);
                    window.location.href = "{{ route('create-checkout-session') }}?price=" + finalPrice;
                } else {
