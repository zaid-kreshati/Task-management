...
class AuthController_api extends Controller_api
{
  ...
  public function register_user(registerRequest $request)
{
    ...
    public function register_leader(registerRequest $request)
    {
        try {
            ...
            // Call the AuthService to register the user
            $result = $this->authService->register($data);
            $user = $result['user']; // For 'register_leader'
            $user->assignRole('leader');

            // Check if registration was successful
            if ($result['status'] === 'success') {
                return $this->successResponse(null, 'Leader registration successful. Welcome!');
            } else {
                return $this->errorResponse('Leader registration failed. Please try again.');
            }
        } catch (ValidationException $e) {
            $errors = $e->errors();
